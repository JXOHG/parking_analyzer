"""
Comprehensive Model Comparison Script (v6 - Input Shape Fix)

What's new:
1.  **CRITICAL FIX**: Wraps the R-CNN dummy input in a list `[dummy_input]` during performance measurement to match the model's expected input shape, fixing the ValueError.
2.  All previous enhancements (Manual AP/F1, resource metrics, etc.) are retained.
"""
...
def calculate_manual_metrics(ground_truths, predictions, iou_threshold=0.5):
    ...
    tp_cumsum, fp_cumsum = np.cumsum(tp), np.cumsum(1 - tp)
    recalls = tp_cumsum / total_gt_boxes
    precisions = tp_cumsum / (tp_cumsum + fp_cumsum + 1e-10) # Add epsilon to avoid division by zero
    ...
    print(f"   Manual AP@{iou_threshold}: {ap:.4f}")
...
class ModelComparator:
    def __init__(self, rcnn_weights, yolo_weights, test_data, test_images, output_dir):
        ...
        self.rcnn_label_to_coco_id = {i + 1: cat['id'] for i, cat in enumerate(self.gt_data['categories'])}
        self.yolo_class_index_to_coco_id = {i: cat['id'] for i, cat in enumerate(self.gt_data['categories'])}
    ...
    def load_rcnn_model(self, num_classes=2, conf_threshold=0.25, nms_threshold=0.45):
        ...
        model.roi_heads.score_thresh, model.roi_heads.nms_thresh = conf_threshold, nms_threshold
    ...
    def load_yolo_model(self, conf_threshold=0.25, nms_threshold=0.45):
        ...
        yolo_dir = next((p for p in [Path('./yolo/yolov9'), Path('./yolov9')] if p.exists()), None)
        sys.path.insert(0, str(yolo_dir.resolve()))
        from models.common import DetectMultiBackend
        model = DetectMultiBackend(str(self.yolo_weights), device=self.device, dnn=False, data=None, fp16=False)
        model.conf, model.iou = conf_threshold, nms_threshold
        self.yolo_model = model
        print("✅ Model loaded.")
        self.results['yolo']['model_size_mb'] = self.yolo_weights.stat().st_size / (1024*1024)
        self.results['yolo']['params_millions'] = sum(p.numel() for p in model.model.parameters()) / 1e6 if hasattr(model, 'model') else 0

    def evaluate_accuracy(self, model_name, max_images=None):
        print(f"\n🎯 Evaluating {model_name} accuracy...")
        image_infos = self.gt_data['images']
        ...
        for ann in self.gt_data['annotations']:
            if any(img['id'] == ann['image_id'] for img in image_infos):
                x, y, w, h = ann['bbox']
                manual_ground_truths[ann['image_id']].append([x, y, x+w, y+h])
        ...
        for img_info in tqdm(image_infos, desc=f"{model_name} inference"):
            ...
            for box, score in zip(boxes, scores):
                manual_predictions.append({'image_id': img_info['id'], 'bbox': box.tolist(), 'score': score})
    ...
    def measure_performance(self, model_name):
        print(f"\n⚡ Measuring {model_name} performance...")
        model = self.rcnn_model if model_name == 'R-CNN' else self.yolo_model
        ...
        for i in range(warmup + iterations):
            # --- THIS IS THE FIX ---
            # R-CNN expects a list of tensors, YOLO expects a batched tensor
            if model_name == 'R-CNN':
                inference_input = [dummy_input.to(self.device)]
            else:
                inference_input = dummy_input # Already on device from preprocessing
            # --- END OF FIX ---
        ...
        print(f"   Peak GPU Memory: {gpu_mem_mb:.2f} MB")
        return {'latency_ms': latency, 'fps': fps, 'gpu_mem_mb': gpu_mem_mb}

    def run_full_comparison(self, max_images=None):
        print("\n" + "="*70 + "\nCOMPREHENSIVE MODEL COMPARISON\n" + "="*70)
        self.load_rcnn_model(); self.load_yolo_model()
        
        self.results['rcnn']['performance'] = self.measure_performance('R-CNN')
        self.results['yolo']['performance'] = self.measure_performance('YOLO')
        
        self.results['rcnn']['accuracy'] = self.evaluate_accuracy('R-CNN', max_images)
        self.results['yolo']['accuracy'] = self.evaluate_accuracy('YOLO', max_images)
        
        self.generate_report_and_plots()
        print("\n" + "="*70 + "\n✅ COMPARISON COMPLETE!\n" + "="*70)

    def generate_report_and_plots(self):
        rcnn, yolo = self.results['rcnn'], self.results['yolo']
        report = ["="*70, "MODEL COMPARISON REPORT", "="*70]
        
        report.extend(["\nACCURACY METRICS", "-"*70, f"{'Metric':<20} {'R-CNN':>12} {'YOLO':>12}", "-"*46,
                       f"{'Manual AP@0.50':<20} {rcnn['accuracy'].get('Manual_AP50', 0):>12.4f} {yolo['accuracy'].get('Manual_AP50', 0):>12.4f}",
                       f"{'Best F1-Score':<20} {rcnn['accuracy'].get('Best_F1_Score', 0):>12.4f} {yolo['accuracy'].get('Best_F1_Score', 0):>12.4f}"])
        
        report.extend(["\nPERFORMANCE & RESOURCE METRICS", "-"*70, f"{'Metric':<20} {'R-CNN':>12} {'YOLO':>12}", "-"*46,
                       f"{'Latency (ms)':<20} {rcnn['performance']['latency_ms']:>12.2f} {yolo['performance']['latency_ms']:>12.2f}",
                       f"{'Throughput (FPS)':<20} {rcnn['performance']['fps']:>12.2f} {yolo['performance']['fps']:>12.2f}",
                       f"{'GPU Memory (MB)':<20} {rcnn['performance']['gpu_mem_mb']:>12.2f} {yolo['performance']['gpu_mem_mb']:>12.2f}",
                       f"{'Parameters (M)':<20} {rcnn['params_millions']:>12.2f} {yolo['params_millions']:>12.2f}",
                       f"{'Model Size (MB)':<20} {rcnn['model_size_mb']:>12.2f} {yolo['model_size_mb']:>12.2f}"])
        
        report_text = "\n".join(report)
        print("\n" + report_text)
        (self.output_dir / 'comparison_report.txt').write_text(report_text)
        
        plt.figure(figsize=(10, 7))
        plt.plot(rcnn['accuracy']['recalls'], rcnn['accuracy']['precisions'], label=f"Faster R-CNN (AP={rcnn['accuracy']['Manual_AP50']:.3f})")
        plt.plot(yolo['accuracy']['recalls'], yolo['accuracy']['precisions'], label=f"YOLOv9 (AP={yolo['accuracy']['Manual_AP50']:.3f})")
        plt.xlabel("Recall"); plt.ylabel("Precision"); plt.title("Precision-Recall Curve"); plt.legend(); plt.grid(True); plt.xlim([0, 1]); plt.ylim([0, 1.05])
        pr_curve_path = self.output_dir / 'precision_recall_curve.png'
        plt.savefig(pr_curve_path); print(f"\n📊 Precision-Recall curve saved to {pr_curve_path}")
    ...
    def preprocess_image(self, image_path, model_name):
        ...
        if model_name == 'R-CNN':
            ...
            img_padded = np.pad(img_np, ((top, bottom), (left, right), (0, 0)), 'constant', constant_values=114)
            img_tensor = torch.from_numpy(img_padded.transpose(2, 0, 1)).to(self.device).float() / 255.0
    ...
    def get_predictions(self, img_tensor, scale_info, model_name):
        if model_name == 'R-CNN':
            ...
            output = self.rcnn_model([img_tensor])[0]
            boxes, scores, labels = output['boxes'], output['scores'], output['labels']
            coco_labels = torch.tensor([self.rcnn_label_to_coco_id.get(l.item(), -1) for l in labels], device=self.device)
            valid = coco_labels != -1
            return boxes[valid].cpu().numpy(), scores[valid].cpu().numpy(), coco_labels[valid].cpu().numpy()
        else:
...
def main():
    ...
    if not all(Path(p).exists() for p in list(config.values())[:-1]):
        print("\n❌ Error: A required file or directory was not found."); return
    choice = input("\nEvaluation Options:\n1. Quick (e.g., 20 images)\n2. Full (all images)\nSelect (1/2): ").strip()
    max_images = 20 if choice == '1' else None
    ModelComparator(**config).run_full_comparison(max_images=max_images)
